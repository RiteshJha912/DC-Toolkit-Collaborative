      ...
      return Object.entries(obj)
        .map(([key, value]) => {
          if (Array.isArray(value)) {
            // Special case to handle tweets list
            if (key === "Recent Tweets") {
              return `<p><strong>${key}:</strong></p>` + value.map(tweet => `<p>${tweet["Created At"]}: ${tweet["Tweet"]}</p>`).join('')
            }
          ...
          return `<p><strong>${key}:</strong> ${value !== null ? value : 'N/A'}</p>`
        })
        .join('')
    }
